...
# 🌟 ถอยกลับมาใช้สูตร V.28 (ทิ้ง PUA) เพื่อรักษาฟอนต์ Kanit ให้สวยงาม
...
# 🌟 V.55.1: อัปเกรดระบบดาวน์โหลด ปลอมตัวเป็นเบราว์เซอร์ + ดึงจาก Github (กันเซิร์ฟเวอร์บล็อก)
def download_custom_engine():
    with st.spinner("⏳ กำลังดาวน์โหลดเอนจิน (ขนาดไฟล์ประมาณ 80MB กรุณารอสักครู่)..."):
        try:
            # จำลองตัวเองเป็นเบราว์เซอร์ Chrome เพื่อไม่ให้เว็บต้นทางบล็อกบอท
            headers = {
                'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Safari/537.36'
            }
            
            # เปลี่ยนไปดึงจาก Github แทน (เสถียรที่สุด)
            url = "https://github.com/BtbN/FFmpeg-Builds/releases/download/latest/ffmpeg-master-latest-linux64-gpl.tar.xz"
            r = requests.get(url, headers=headers, stream=True, allow_redirects=True)
            
            # ถ้า Github ล่ม ให้กลับไปใช้ John Van Sickle
            ...
            # เช็กว่าไฟล์ที่โหลดมาไม่ใช่ไฟล์ Error หน้าเว็บ (ถ้าน้อยกว่า 1MB คือพังแน่นอน)
...
# คืนชีพปุ่มดาวน์โหลดเอนจินแบบ V.28 ไว้ที่ Sidebar
...
if app_mode == "🎭 โหมด 1: สร้างคลิปไร้หน้า (Faceless)":
    ...
    if st.session_state.m1_segments:
        ...
        if st.button("🎬 ขั้นตอนที่ 2: สร้างพรีวิววิดีโอ", use_container_width=True, type="primary"):
            ...
            # 🌟 เช็กและติดตั้งเอนจินอัตโนมัติก่อนเรนเดอร์ทุกครั้ง (กันพลาด)
            ...
            with st.spinner("กำลังประกอบร่างวิดีโอ..."):
                ...
                for seg in segments_to_process:
                    ...
                    # 🌟 ใช้ฟังก์ชันดั้งเดิม เพื่อรักษา Kanit Bold
