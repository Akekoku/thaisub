...
def load_profiles():
    default_profiles = {
        ...
        "🎞️ สไตล์ Cinematic (เรียบหรู กล่องดำ)": {
            ...
            "fs": 16, "ot": 0, "mw": 100, "mv": 30, "ll": "ไม่จำกัด (ตามความกว้าง)", "ai_proof": True
        },
...
def render_subtitle_ui(key_prefix):
    defaults = {
        ...
        "fs": 18, "ot": 1, "mw": 100, "mv": 50, "ll": "🚫 บังคับไม่เกิน 2 บรรทัด", "ai_proof": True 
    }
    ...
    with st.expander("เปิดเครื่องมือปรับแต่ง (ฟอนต์, เอฟเฟกต์, AI)", expanded=True):
        st.markdown("#### 💾 จัดการโปรไฟล์ซับไตเติล")
        ...
        st.markdown("#### 🧠 ผู้ช่วย AI ตรวจคำผิดตามบริบท (V2 - โหมดบังคับ)")
        ai_proof_choice = st.checkbox("เปิดใช้งาน AI พิสูจน์อักษรแบบเคร่งครัด (แก้เฉพาะคำพ้องเสียง ห้ามเปลี่ยนคำศัพท์)", key=f"{key_prefix}_ai_proof")
        st.markdown("---")
        
        st.markdown("#### 📝 แก้ไขคำทับศัพท์ / เปลี่ยนคำผิด (ช่วยใบ้คำให้ AI)")
        ...
        return {
            ...
            "raw_replace_input": replace_words_input
        }
        ...
        with st.spinner("กำลังทำงาน..."):
            ...
            if sub_config["ai_proof"]:
                with st.spinner("🧠 กำลังให้ AI ตรวจทานคำผิดแบบจำกัดกรอบ..."):
                    ...
                    if ai_success: 
                        st.toast("✨ AI แก้ไขคำผิดสำเร็จ!")
                    else: 
                        st.toast("⚠️ มีข้อผิดพลาดบางส่วน! ดู Logs")
                        for idx, log in enumerate(debug_logs):
                            with st.expander(f"🔍 ดูข้อผิดพลาด AI (จุดที่ {idx+1})", expanded=True):
            ...
            if faceless_mode != "ไม่ใช้ (ใช้วิดีโอต้นฉบับ)":
                ...
                subprocess.run([FFMPEG_CMD, '-y', '-i', 'raw_upload.mp4', '-vf', 'subtitles=subs.ass:fontsdir=.', '-c:v', 'libx264', '-crf', '18', '-preset', 'fast', '-c:a', 'aac', '-b:a', '256k', 'output.mp4'])
...
# =========================================================
    ...
    if uploaded_videos and st.button("🚀 รวมคลิปและสร้างซับไตเติล"):
        ...
        with st.spinner("กำลังฟังเสียงและถอดความ..."):
            ...
            subprocess.run([FFMPEG_CMD, '-y', '-i', 'combined.mp4', '-vf', 'subtitles=subs_joined.ass:fontsdir=.', '-c:v', 'libx264', '-crf', '18', '-preset', 'fast', '-c:a', 'aac', '-b:a', '256k', 'output_joined.mp4'])
