...
# 🌟 คืนชีพกล่องเครื่องมือดาวน์โหลด FFmpeg (ซ่อมสระลอยของแท้) ตาม V.28
st.sidebar.markdown("---")
...
if st.sidebar.button("🔄 ติดตั้ง / อัปเกรดเอนจิน (ซ่อมสระลอย)"):
    with st.spinner("⏳ กำลังดาวน์โหลดเอนจินเรนเดอร์รุ่นพิเศษ (ประมาณ 35MB)..."):
        try:
            ...
            with st.spinner("📦 กำลังแตกไฟล์และติดตั้งระบบประกอบสระภาษาไทย..."):
                ...
                # ทำให้ไฟล์รันได้ (Execute permission)
            ...
            st.sidebar.success("✅ ติดตั้งเอนจินเสร็จสมบูรณ์! สระไม่ลอยแล้วครับ")
            time.sleep(2)
            st.rerun()
        except Exception as e:
            st.sidebar.error(f"❌ เกิดข้อผิดพลาดในการติดตั้ง: {e}")

st.sidebar.markdown("---")

# เลือกว่าจะใช้ ffmpeg ตัวเทพที่เราโหลดมา หรือตัวพื้นฐานของระบบ
...
if app_mode == "🎭 โหมด 1: สร้างคลิปไร้หน้า (Faceless)":
    ...
    if st.session_state.m1_segments:
        ...
        if st.button("🎬 ขั้นตอนที่ 2: สร้างพรีวิววิดีโอ", use_container_width=True, type="primary"):
            if not engine_exists:
                st.warning("⚠️ น้าอย่าลืมกดปุ่ม 'ติดตั้ง / อัปเกรดเอนจิน' ที่แถบด้านซ้ายมือขวาก่อนเรนเดอร์นะครับ ไม่งั้นสระจะลอยนะ!")
