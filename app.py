...
def fix_thai_floating_vowels(text):
    ...
    # แยกนิคหิตกับลากข้าง เพื่อช่วยจูนเบื้องต้น
    text = text.replace("ำ", "ํา") 
    return text

# 🌟 ฟังก์ชันดาวน์โหลดเอนจิน (แบบเดียวกับปุ่มใน V.28)
def download_custom_engine():
    with st.spinner("⏳ กำลังดาวน์โหลดเอนจินแก้สระลอย (กรุณารอสักครู่)..."):
        try:
...
def ensure_ffmpeg_engine():
    # ฟังก์ชันแอบเช็กอัตโนมัติก่อนเรนเดอร์ (แก้ปัญหาเล่นผ่านมือถือแล้วพัง)
...
if app_mode == "🎭 โหมด 1: สร้างคลิปไร้หน้า (Faceless)":
    ...
    if st.session_state.m1_segments:
        ...
        if st.button("🎬 ขั้นตอนที่ 2: สร้างพรีวิววิดีโอ", use_container_width=True, type="primary"):
            ...
            # 🌟 อัปเกรด: เช็กและติดตั้งเอนจินอัตโนมัติก่อนเรนเดอร์ทุกครั้ง (กันพลาด)
