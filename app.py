...
# 🌟 ฟังก์ชันใหม่: แก้ปัญหาสระลอย วรรณยุกต์ซ้อนทับกัน
def fix_thai_rendering(text):
    upper_vowels = ['ิ', 'ี', 'ึ', 'ื', 'ั', 'ํ', '็']
    long_tail_cons = ['ป', 'ฝ', 'ฟ']
    up_shift_tones = {'่': '\uf70a', '้': '\uf70b', '๊': '\uf70c', '๋': '\uf70d', '์': '\uf70e'}
    left_shift_vowels = {'ิ': '\uf701', 'ี': '\uf702', 'ึ': '\uf703', 'ื': '\uf704', 'ั': '\uf710', 'ํ': '\uf711', '็': '\uf712'}
    left_shift_tones = {'่': '\uf705', '้': '\uf706', '๊': '\uf707', '๋': '\uf708', '์': '\uf709'}
    left_up_shift_tones = {'่': '\uf713', '้': '\uf714', '๊': '\uf715', '๋': '\uf716', '์': '\uf717'}
    ...
    for i in range(len(text)):
        ...
        if char in ['ุ', 'ู'] and prev_char == 'ญ': res = res[:-1] + '\uf70f' + char
        elif char in ['ุ', 'ู'] and prev_char == 'ฐ': res = res[:-1] + '\uf700' + char
        elif char in left_up_shift_tones and prev_char in upper_vowels and prev2_char in long_tail_cons: res += left_up_shift_tones[char]
        elif char in up_shift_tones and prev_char in upper_vowels: res += up_shift_tones[char]
        elif char in left_shift_vowels and prev_char in long_tail_cons: res += left_shift_vowels[char]
        elif char in left_shift_tones and prev_char in long_tail_cons: res += left_shift_tones[char]
        else: res += char
    return res
    ...
    if uploaded_file and st.button("🚀 เริ่มผลิตคลิปไร้หน้า"):
        with st.spinner("กำลังทำงาน..."):
            ...
            for text_sub, start_time, end_time in segments_data_for_subs:
                ...
                # 🌟 ซ่อมสระลอยและวรรณยุกต์ซ้อน ตรงนี้! 🌟
                formatted_text = fix_thai_rendering(formatted_text)
                ...
                with open("concat.txt", "w") as f:
                    for i, sc in enumerate(scenes):
                        ...
                        if faceless_mode == "🤖 Pexels AI (ดึงภาพอัตโนมัติ)":
                            ...
                            if fetch_pexels_video(kw, pexels_key, "temp.mp4"): subprocess.run(['ffmpeg', '-y', '-i', 'temp.mp4', '-ss', '0', '-t', str(sc_dur), '-vf', f'scale={video_width}:{video_height}:force_original_aspect_ratio=increase,crop={video_width}:{video_height}', '-c:v', 'libx264', '-an', c_path])
            ...
            else: subprocess.run(['ffmpeg', '-y', '-i', 'raw_upload.mp4', '-vf', 'subtitles=subs.ass:fontsdir=.', '-c:v', 'libx264', '-c:a', 'aac', 'output.mp4'])
