...
uploaded_file = st.file_uploader("📂 เลือกไฟล์วีดีโอหรือไฟล์เสียง (MP4 / MP3 / WAV)", type=["mp4", "mp3", "wav"])
    ...
    if st.button("🚀 เริ่มกระบวนการโรงงาน AI อัตโนมัติ"):
       ...
       # จัดการสกัดเสียงออกมาก่อนไม่ว่าอินพุตจะเป็นอะไร
        ...
        if file_ext == "mp3":
            ...
            # ให้ FFmpeg แปลง WAV เป็น MP3 หลังบ้านอัตโนมัติ เพื่อให้ระบบเดิมทำงานต่อได้เนียนๆ
            subprocess.run(['ffmpeg', '-y', '-i', 'raw_upload.wav', '-c:a', 'libmp3lame', '-b:a', '64k', 'audio.mp3'], check=True)
        else:
