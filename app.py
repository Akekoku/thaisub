...
# =========================================================
# 🛠️ ฟังก์ชันสร้างหน้าต่างตั้งค่าซับไตเติลแบบเต็มสูบ
# =========================================================
def render_subtitle_ui(key_prefix):
    ...
    with st.expander("เปิดเครื่องมือปรับแต่ง (ฟอนต์, สี, เอฟเฟกต์, แก้คำผิด)", expanded=True):
        ...
        # --- ส่วนใหม่: ระบบแทนที่คำศัพท์ ---
        st.markdown("#### 📝 แก้ไขคำทับศัพท์ / เปลี่ยนคำผิด")
        replace_words_input = st.text_input(
            "รูปแบบ (คำเดิม=คำใหม่) คั่นด้วยลูกน้ำ:", 
            value="Save=เซฟ, OK=โอเค, AI=เอไอ, Content=คอนเทนต์",
        ...
        force_max_2_lines = st.checkbox("🚫 บังคับซับไม่เกิน 2 บรรทัด (ตัดขึ้นหน้าใหม่)", value=True, key=f"{key_prefix}_f2l")
        
        # จัดเตรียม Dictionary สำหรับการแทนที่คำ
        replace_dict = {}
...
if app_mode == "🎭 โหมด 1: สร้างคลิปไร้หน้า (Faceless)":
    ...
    if uploaded_file and st.button("🚀 เริ่มผลิตคลิปไร้หน้า"):
        with st.spinner("กำลังทำงาน..."):
            ...
            effect_prefix = ""
            if sub_config["anim"] == "เด้งพอง (Pop-up)":
                effect_prefix = f"{{\\fscx{sub_config['pop_scale']}\\fscy{sub_config['pop_scale']}\\t(0,{sub_config['pop_dur']},\\fscx100\\fscy100)}}"
            elif sub_config["anim"] == "ค่อยๆ ปรากฏ (Fade-in)":
                effect_prefix = f"{{\\fad({sub_config['fade_dur']},0)}}"
            ...
            for seg in res_th.segments:
                ...
                # --- แทนที่คำศัพท์ (Word Replacement) ---
                for old_w, new_w in sub_config["replacements"].items():
                    # ใช้ Regex เพื่อให้แทนที่ได้ทั้งตัวพิมพ์เล็ก/ใหญ่แบบไม่สนใจ Case
                    t = re.sub(re.escape(old_w), new_w, t, flags=re.IGNORECASE)
