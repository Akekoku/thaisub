...
# =========================================================
# 🚀 ระบบโหลด FFmpeg ฉบับ Full Option (แก้ปัญหาสระลอย 100%)
# =========================================================
@st.cache_resource
def prepare_ffmpeg():
    if platform.system() == "Linux":
        if not os.path.exists("ffmpeg"):
            with st.spinner("📥 กำลังติดตั้ง Video Engine รุ่นพิเศษสำหรับภาษาไทย (ทำแค่ครั้งแรก)..."):
                try:
                    subprocess.run("wget https://johnvansickle.com/ffmpeg/releases/ffmpeg-release-amd64-static.tar.xz", shell=True, check=True)
                    subprocess.run("tar -xf ffmpeg-release-amd64-static.tar.xz", shell=True, check=True)
                    subprocess.run("cp ffmpeg-*-static/ffmpeg .", shell=True, check=True)
                    subprocess.run("cp ffmpeg-*-static/ffprobe .", shell=True, check=True)
                    subprocess.run("chmod +x ffmpeg ffprobe", shell=True, check=True)
                    subprocess.run("rm -rf ffmpeg-*-static* ffmpeg-release-amd64-static.tar.xz", shell=True)
                except Exception as e:
                    print("Failed to download static ffmpeg:", e)
        if os.path.exists("ffmpeg"):
            return './ffmpeg', './ffprobe'
    return 'ffmpeg', 'ffprobe'

FFMPEG_CMD, FFPROBE_CMD = prepare_ffmpeg()
