...
with st.expander("คลิกเพื่อเปิดเครื่องมือปรับแต่งตัวอักษรและการตัดคำ", expanded=True):
    ...
    # index=1 คือค่าเริ่มต้นที่ 6 คำ (นับจาก 0, 1, 2...)
    max_words_choice = st.selectbox("🔤 จำนวนคำสูงสุดต่อ 1 บรรทัด:", [5, 6, 7, 8], index=1)
    
    # index=8 คือค่าเริ่มต้นที่ฟอนต์ "Sarabun" (ตัวที่ 9 ในวงเล็บ)
    ...
    with col1:
        # value=18 คือตั้งค่าเริ่มต้นสไลเดอร์ไว้ที่ 18
        font_size_choice = st.slider("📏 ขนาดตัวอักษร (FontSize):", min_value=14, max_value=40, value=18)
    with col2:
        # value=0 คือตั้งค่าเริ่มต้นสไลเดอร์ไว้ที่ 0
        line_gap_choice = st.slider("↕️ ระยะห่างระหว่างบรรทัด (Line Gap):", min_value=0, max_value=30, value=0)
        
    # value=50 คือตั้งค่าเริ่มต้นสไลเดอร์ไว้ที่ 50
