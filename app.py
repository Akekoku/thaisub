...
def load_profiles():
    ...
    # 🌟 โปรไฟล์เริ่มต้น 3 แบบ (อัปเดตให้รองรับแบบ 1 บรรทัด)
    default_profiles = {
        "🎬 สไตล์ Tiktok (เด้งพอง ตัวเหลือง)": {
            ...
            "fs": 22, "ot": 2, "mw": 90, "mv": 50, "ll": "🚫 บังคับ 1 บรรทัด"
        },
        "🎞️ สไตล์ Cinematic (เรียบหรู กล่องดำ)": {
            ...
            "fs": 16, "ot": 0, "mw": 100, "mv": 30, "ll": "ไม่จำกัด (ตามความกว้าง)"
        },
        "🎀 สไตล์ Vlog (น่ารัก ฟอนต์ลายมือ)": {
            ...
            "fs": 20, "ot": 2, "mw": 80, "mv": 40, "ll": "🚫 บังคับไม่เกิน 2 บรรทัด"
        }
    }
...
def render_subtitle_ui(key_prefix):
    ...
    defaults = {
        ...
        "fs": 18, "ot": 1, "mw": 100, "mv": 50, "ll": "🚫 บังคับไม่เกิน 2 บรรทัด"
    }
    
    # ระบบรองรับโปรไฟล์เก่า (ถ้าโปรไฟล์เก่ามีแค่ f2l ให้แปลงเป็น ll)
    profiles = load_profiles()
    for p_name, p_data in profiles.items():
        if "ll" not in p_data and "f2l" in p_data:
            p_data["ll"] = "🚫 บังคับไม่เกิน 2 บรรทัด" if p_data["f2l"] else "ไม่จำกัด (ตามความกว้าง)"
    ...
    with st.expander("เปิดเครื่องมือปรับแต่ง (ฟอนต์, สี, เอฟเฟกต์, จัดการโปรไฟล์)", expanded=True):
            ...
            if st.button("📥 โหลดมาใช้", key=f"{key_prefix}_load_btn"):
                if selected_prof != "-- เลือกโปรไฟล์เพื่อโหลด --":
                    for k, v in profiles[selected_prof].items():
                        if k in defaults: # ป้องกัน key ขยะ
                            st.session_state[f"{key_prefix}_{k}"] = v
                    st.rerun()
        ...
        return {
            ...
            "line_limit": line_limit_choice, "replacements": replace_dict
        }
