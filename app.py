...
def clean_whisper_segments(segments):
    ...
    for seg in segments:
        ...
        is_hallucination = (ratio > 0.85 or text == last_text) and (time_gap < 1.0)
...
# 🌟 V.36 ปรับบริบท AI พิสูจน์อักษรให้ฉลาดรู้เรื่อง IT และกันคำสุ่มเสี่ยง
def ai_proofread_segments(client, segments, user_replacements=""):
    ...
    for i in range(0, len(segments), chunk_size):
        ...
        prompt = f"""คุณคือ AI ผู้เชี่ยวชาญการแก้ไขคำผิดจากระบบถอดเสียง (Speech-to-Text) ภาษาไทย
บริบทของเสียงนี้คือ: "การแก้ปัญหาไอที, สมาร์ทโฟน, แอปธนาคาร, การตั้งค่ามือถือ"

หน้าที่ของคุณ: ตรวจสอบและแก้ไข "คำที่ AI ฟังเพี้ยน" ให้กลับมาเป็นภาษาพูดที่ถูกต้องตามบริบทไอที

⚠️ กฎเหล็ก (สำคัญมาก):
- ระวังคำสุ่มเสี่ยงที่ทำให้โดนแบน แพลตฟอร์ม: "นักพนัน" หรือ "นักพนันทิ้งไว้" ให้แก้เป็น "โหมดนักพัฒนา" หรือ "นักพัฒนา" ทันที!
- คำเพี้ยนจากความเร็วที่ต้องแก้: 
  - "วิทย์ที่แก้เงี่ยงๆ" หรือ "วิธีแก้เงี่ยงๆ" -> "วิธีแก้ง่ายๆ"
  - "ปั๊ก 2" หรือ "พรรค 2" -> "ข้อ 2" หรือ "อย่างที่ 2"
  - "App หื่อน" -> "App เถื่อน"
  - "อุดรูรวบ" -> "อุดรูรั่ว"
  - "ไม่ปั๊ม" -> "ไม่ปลื้ม"

กฎการทำงาน:
1. แก้ไขคำเพี้ยนตามตัวอย่างและบริบทไอทีด้านบน
2. ห้ามแต่งประโยคใหม่ ห้ามลบประโยคทิ้ง คงโครงสร้างเดิมไว้
3. ตอบกลับเป็น JSON Object เท่านั้น รูปแบบ: {{"corrected": [{{"id": "0", "text": "ข้อความ"}}]}}
{hint_text}
ข้อความต้นฉบับ:
...
if app_mode == "🎭 โหมด 1: สร้างคลิปไร้หน้า (Faceless)":
    ...
    if uploaded_file and st.button("🚀 เริ่มผลิตคลิปไร้หน้า"):
        with st.spinner("กำลังทำงาน..."):
            ...
            # 🌟 V.36 ป้อนศัพท์ใส่สมอง AI ตั้งแต่เริ่มฟังเสียง
            with open("audio.mp3", "rb") as audio_file:
                res_th = client.audio.transcriptions.create(
                    ...
                    language="th", 
                    temperature=0.0,
                    prompt="แอปธนาคาร โอนเงินไม่ได้ โหมดนักพัฒนา นักพัฒนา แอปเถื่อน วิธีแก้ง่ายๆ อุดรูรั่ว ไม่ปลื้ม ยอดฮิต ข้อ 1 ข้อ 2 ข้อ 3 ไอที เทคโนโลยี สมาร์ทโฟน"
                )
            ...
            if sub_config["ai_proof"]:
                with st.spinner("🧠 กำลังให้ AI ตรวจทานคำผิดและเฝ้าระวังคำสุ่มเสี่ยง..."):
            ...
            if faceless_mode == "ไม่ใช้ (ใช้วิดีโอต้นฉบับ)" and sub_config["auto_cut"] == "เปิด (ตัดช่วงเงียบอัตโนมัติ)":
                with st.spinner("✂️ กำลังตัดเดดแอร์แบบละเอียดระดับเฟรม (Frame-Accurate)..."):
                    ...
                    for i, seg in enumerate(raw_segments):
                        ...
                        e_cut = min(orig_dur_process, seg['end'] + pad_e) if orig_dur_process > 0 else seg['end'] + pad_e
                    ...
                    if merged_times and orig_dur_process > 0:
                        if orig_dur_process - merged_times[-1]['end'] <= 4.0:
                            merged_times[-1]['end'] = orig_dur_process
