...
import tarfile
import json
import difflib  # 🌟 V.33 เพิ่มไลบรารีสแกนคำซ้ำ
...
def load_profiles():
    default_profiles = {
        ...
        "🎀 สไตล์ Vlog (น่ารัก ฟอนต์ลายมือ)": {
            ...
            "fs": 20, "ot": 2, "mw": 80, "mv": 40, "ll": "🚫 บังคับไม่เกิน 2 บรรทัด", "ai_proof": True, "auto_cut": "ปิด (เก็บช่วงเงียบไว้ปกติ)"
        }
    }
...
# 🌟 V.33: อัปเกรดเครื่องสแกนและปราบผี AI หูแว่ว (Bulletproof Anti-Hallucination)
def clean_whisper_segments(segments):
    ...
    for seg in segments:
        ...
        # กรองคำขยะที่ AI ชอบหลอนออกมาเอง
        if text in ["ซับไทยโดย GDH", "แปลซับโดย", "ดนตรี", "เสียงดนตรี"]: continue
        ...
        # ตรวจสอบความเหมือนของข้อความ
        seq = difflib.SequenceMatcher(None, text, last_text)
        ratio = seq.ratio()
        
        # ตรวจสอบว่าข้อความใหม่ เป็นส่วนหนึ่งของประโยคเก่าหรือไม่ (เช่น "หอพัก คอนโด" มักจะหลอนมาจาก "วางในหอพัก คอนโด")
        is_substring = (text in last_text) or (last_text in text)
        is_long_enough = len(text) > 3 and len(last_text) > 3
        
        time_gap = seg['start'] - last_seg['end']

        # ถ้าความหมายเหมือนกันเกิน 55% หรือเป็นคำเดียวกันซ้ำๆ และเวลาติดกัน (ห่างกันไม่เกิน 3 วินาที) = AI หลอนชัวร์!
        is_hallucination = (ratio > 0.55 or (is_substring and is_long_enough)) and (time_gap < 3.0)

        if is_hallucination:
            # จับเวลามารวมกัน เพื่อไม่ให้วิดีโอต้องกระโดดตัดไปมา
            last_seg['end'] = max(last_seg['end'], seg['end'])
            # เลือกประโยคที่ยาวกว่ามาเป็นซับไตเติล (เพราะมักจะสมบูรณ์กว่า)
            if len(text) > len(last_text):
                last_seg['text'] = text
            continue
            
        # ป้องกันเรื่องเวลาย้อนกลับ (Time-Paradox)
...
def ai_proofread_segments(client, segments, user_replacements=""):
    ...
    if user_replacements:
        hint_text = f"\nคำใบ้เพิ่มเติม (ผู้ใช้ต้องการให้แก้คำเหล่านี้ถ้าเจอ): {user_replacements}\n"

    for i in range(0, len(segments), chunk_size):
        chunk = segments[i:i+chunk_size]
        data_to_fix = [{"id": str(idx), "text": seg["text"]} for idx, seg in enumerate(chunk)]
        
        prompt = f"""คุณคือบอทพิสูจน์อักษรภาษาไทยจากไฟล์เสียง (Speech-to-Text)
หน้าที่ของคุณมีเพียงอย่างเดียว: แก้ไข "คำพ้องเสียงที่สะกดผิด" ให้ถูกต้องตามบริบทแวดล้อม (เช่น รถ/ลด, หน้า/น่า, คะ/ค่ะ, กับ/กลับ)

ข้อห้ามเด็ดขาด (Strict Rules):
1. ห้าม Paraphrase หรือเปลี่ยนคำศัพท์ไปใช้คำอื่นเด็ดขาด
2. ห้ามแต่งประโยคใหม่ ห้ามรวมประโยค ห้ามลบคำทิ้ง
3. หากข้อความนั้นถูกต้องอยู่แล้ว ให้คงข้อความเดิมไว้ 100% ห้ามแก้ไขใดๆ ทั้งสิ้น
4. ตอบกลับเป็น JSON Object เท่านั้น รูปแบบ: {{"corrected": [{{"id": "0", "text": "ข้อความ"}}]}}
{hint_text}
ข้อความต้นฉบับ:
{json.dumps(data_to_fix, ensure_ascii=False)}"""
        
        chunk_success = False
        last_error_or_content = ""
        ...
        for attempt in range(3):
            try:
                ...
                if isinstance(corrected_list, list) and len(corrected_list) == len(data_to_fix):
                    ...
                    last_error_or_content = f"⚠️ ปัญหา: จำนวนบรรทัดไม่ตรงกัน!\nส่งไป: {len(data_to_fix)} บรรทัด\nตอบกลับมา: {len(corrected_list)} บรรทัด"
                    time.sleep(2)
            except Exception as e:
                last_error_or_content = f"🔥 ปัญหา: {str(e)}"
                time.sleep(3)
                
        if not chunk_success:
            success_all = False
            debug_logs.append(last_error_or_content)
...
if st.sidebar.button("🔄 ติดตั้ง / อัปเกรดเอนจิน (ซ่อมสระลอย)"):
    with st.spinner("กำลังดาวน์โหลด FFmpeg รุ่น Full-Engine..."):
        try:
            ...
            with tarfile.open("ffmpeg.tar.xz", "r:xz") as tar: tar.extractall()
            ...
            os.chmod("./ffmpeg", 0o755); os.chmod("./ffprobe", 0o755)
            os.remove("ffmpeg.tar.xz"); shutil.rmtree(extracted)
            st.sidebar.success("✅ อัปเกรดเสร็จสมบูรณ์!")
...
def render_subtitle_ui(key_prefix):
    ...
    st.markdown("### 🛠️ ปรับแต่งสไตล์ซับไตเติล & วิดีโอ")
    with st.expander("เปิดเครื่องมือปรับแต่ง (ฟอนต์, เอฟเฟกต์, AI, การตัดต่อ)", expanded=True):
        st.markdown("#### 💾 จัดการโปรไฟล์")
        ...
        with col_save:
            new_prof_name = st.text_input("💾 บันทึกการตั้งค่าปัจจุบันเป็นโปรไฟล์ใหม่:", placeholder="ตั้งชื่อโปรไฟล์...", key=f"{key_prefix}_new_prof")
            ...
            with c_d:
                if st.button("🗑️ ลบโปรไฟล์ที่เลือก", key=f"{key_prefix}_del_btn"):
                    if selected_prof != "-- เลือกโปรไฟล์เพื่อโหลด --" and selected_prof in profiles:
                        ...
                        st.success("ลบโปรไฟล์เรียบร้อย!"); time.sleep(1); st.rerun()
        
        st.markdown("---")
        st.markdown("#### ☁️ สำรองข้อมูลโปรไฟล์ (กันไฟล์หายตอนปิดเว็บ)")
        ce1, ce2 = st.columns(2)
        with ce1:
            if os.path.exists(PROFILE_FILE):
                with open(PROFILE_FILE, "rb") as f:
                    st.download_button("📤 1. ดาวน์โหลดโปรไฟล์เก็บไว้ในคอม", f, file_name="my_profiles.json", mime="application/json", key=f"{key_prefix}_export")
        with ce2:
            uploaded_prof = st.file_uploader("📥 2. นำเข้าไฟล์โปรไฟล์ที่เคยโหลดไว้", type=["json"], key=f"{key_prefix}_import")
            if uploaded_prof:
                if st.button("ตกลงอัปเดตเข้าระบบ", key=f"{key_prefix}_do_import"):
                    try:
                        imported_data = json.loads(uploaded_prof.getvalue().decode("utf-8"))
                        profiles.update(imported_data)
                        save_profiles(profiles)
                        st.success("นำเข้าสำเร็จ! กำลังรีเฟรช..."); time.sleep(1); st.rerun()
                    except: st.error("ไฟล์ไม่ถูกต้อง!")

        st.markdown("---")
        st.markdown("#### ✂️ ระบบผู้ช่วยตัดต่อวิดีโอ (Jumpcut AI)")
        auto_cut_choice = st.radio("เลือกรูปแบบการจัดการคลิป:", ["ปิด (เก็บช่วงเงียบไว้ปกติ)", "เปิด (ตัดช่วงเงียบอัตโนมัติ)"], horizontal=True, key=f"{key_prefix}_ac")
        st.caption("✨ หากเปิดใช้งาน: ระบบจะหั่นเฉพาะช่วงที่มีเสียงพูดมาต่อกัน ช่วยลบ Dead Air ให้คลิปกระชับขึ้นอัตโนมัติ")

        st.markdown("---")
        st.markdown("#### 🧠 ผู้ช่วย AI ตรวจคำผิดตามบริบท")
        ai_proof_choice = st.checkbox("เปิดใช้งาน AI พิสูจน์อักษรแบบเคร่งครัด", key=f"{key_prefix}_ai_proof")
        replace_words_input = st.text_input("📝 แก้ไขคำทับศัพท์ (คำเดิม=คำใหม่ คั่นด้วยลูกน้ำ):", key=f"{key_prefix}_replace")
        ...
        st.markdown("#### 📏 การจำกัดบรรทัดซับไตเติล")
        line_options = ["ไม่จำกัด (ตามความกว้าง)", "🚫 บังคับไม่เกิน 2 บรรทัด", "🚫 บังคับ 1 บรรทัด"]
        line_limit_choice = st.radio("เลือกรูปแบบการแสดงผล:", line_options, horizontal=True, key=f"{key_prefix}_ll")
        ...
        return {
            ...
            "pop_scale": st.session_state[f"{key_prefix}_ps"], "pop_dur": st.session_state[f"{key_prefix}_pd"], 
            "fade_dur": st.session_state[f"{key_prefix}_fd"], "size": font_size_choice, 
...
if app_mode == "🎭 โหมด 1: สร้างคลิปไร้หน้า (Faceless)":
    ...
    if uploaded_file and st.button("🚀 เริ่มผลิตคลิปไร้หน้า"):
        with st.spinner("กำลังทำงาน..."):
            ...
            # 🌟 V.33 เพิ่ม Prompt ดักคอ AI ไม่ให้แต่งเรื่องเอง
            with open("audio.mp3", "rb") as audio_file:
                res_th = client.audio.transcriptions.create(
                    model="whisper-large-v3", 
                    file=("audio.mp3", audio_file), 
                    response_format="verbose_json", 
                    language="th", 
                    temperature=0.0,
                    prompt="กรุณาถอดเสียงพูดภาษาไทยให้ถูกต้องแม่นยำ ห้ามพิมพ์ข้อความซ้ำกัน ห้ามแต่งประโยคเอง ถอดความตามเสียงที่ได้ยินเท่านั้น"
                )
            ...
            for seg in res_th.segments:
                ...
                t = seg['text'].strip() if isinstance(seg, dict) else seg.text.strip()
                if t: raw_segments.append({"start": s, "end": e, "text": t})
                
            raw_segments = clean_whisper_segments(raw_segments)

            if sub_config["ai_proof"]:
                with st.spinner("🧠 กำลังให้ AI ตรวจทานคำผิดแบบจำกัดกรอบ..."):
                    raw_segments, ai_success, debug_logs = ai_proofread_segments(client, raw_segments, sub_config["raw_replace_input"])
                    if ai_success: st.toast("✨ AI แก้ไขคำผิดสำเร็จ!")
                    else: st.toast("⚠️ มีข้อผิดพลาดบางส่วน!")

            video_to_process = "raw_upload.mp4"
            ...
            if faceless_mode == "ไม่ใช้ (ใช้วิดีโอต้นฉบับ)" and sub_config["auto_cut"] == "เปิด (ตัดช่วงเงียบอัตโนมัติ)":
                with st.spinner("✂️ กำลังตัดเดดแอร์แบบละเอียดระดับเฟรม (Frame-Accurate)..."):
                    merged_times = []
                    for i, seg in enumerate(raw_segments):
                        pad_s = 0.15 
                        pad_e = 0.15
                        s_cut = max(0.0, seg['start'] - pad_s)
                        e_cut = seg['end'] + pad_e
                        
                        if not merged_times:
                            merged_times.append({'start': s_cut, 'end': e_cut, 'segments': [seg]})
                        else:
                            last_block = merged_times[-1]
                            if s_cut <= last_block['end']: 
                                last_block['end'] = max(last_block['end'], e_cut)
                                last_block['segments'].append(seg)
                            else:
                                merged_times.append({'start': s_cut, 'end': e_cut, 'segments': [seg]})
                    ...
                    if merged_times:
                        ...
                        for block in merged_times:
                            block_s = block['start']
                            block_e = block['end']
                            block_dur = block_e - block_s
                            for seg in block['segments']:
                                rel_s = max(0.0, seg['start'] - block_s)
                                rel_e = min(block_dur, seg['end'] - block_s)
                                new_segments.append({
                                    'start': current_new_time + rel_s,
                                    'end': current_new_time + rel_e,
                                    'text': seg['text']
                                })
                            current_new_time += block_dur
            ...
            limit_choice = sub_config["line_limit"]
            chunk_size = 0
            if "2 บรรทัด" in limit_choice: chunk_size = 2
            elif "1 บรรทัด" in limit_choice: chunk_size = 1
            ...
            for text_sub, start_time, end_time in segments_data_for_subs:
                ...
                if chunk_size > 0 and len(lines) > chunk_size:
                    ...
                    ass_content += f"Dialogue: 0,{format_ass_timestamp(start_time)},{format_ass_timestamp(end_time)},Default,,0,0,0,,{effect_prefix}{formatted_text.replace('\n', '\\N')}\n"
                ...
                with open("concat.txt", "w") as f:
                    for i, sc in enumerate(scenes):
                        ...
                        if faceless_mode == "🤖 Pexels AI (ดึงภาพอัตโนมัติ)":
                            ...
                            if fetch_pexels_video(kw, pexels_key, "temp.mp4"): subprocess.run([FFMPEG_CMD, '-y', '-i', 'temp.mp4', '-ss', '0', '-t', str(sc_dur), '-vf', f'scale={video_width}:{video_height}:force_original_aspect_ratio=increase,crop={video_width}:{video_height}', '-c:v', 'libx264', '-crf', '18', '-preset', 'fast', '-an', c_path])
                            ...
                            if in_b: subprocess.run([FFMPEG_CMD, '-y', '-i', in_b, '-ss', '0', '-t', str(sc_dur), '-vf', f'scale={video_width}:{video_height}:force_original_aspect_ratio=increase,crop={video_width}:{video_height}', '-c:v', 'libx264', '-crf', '18', '-preset', 'fast', '-an', c_path])
                            else: subprocess.run([FFMPEG_CMD, '-y', '-f', 'lavfi', '-i', f'color=c=black:s={video_width}x{video_height}:d={sc_dur}', '-c:v', 'libx264', '-crf', '18', '-preset', 'fast', c_path])
        ...
        if faceless_mode == "ไม่ใช้ (ใช้วิดีโอต้นฉบับ)":
            st.markdown("---")
            st.markdown("### 📊 สรุปข้อมูลคลิป (Video Report)")
            orig_size = get_file_size_mb("raw_upload.mp4")
            orig_dur = get_video_duration("raw_upload.mp4")
            new_size = get_file_size_mb("output.mp4")
            new_dur = get_video_duration("output.mp4")
            
            rc1, rc2, rc3 = st.columns(3)
            rc1.metric("⏱️ ความยาว (วินาที)", f"{new_dur:.1f} s", f"{new_dur - orig_dur:.1f} s" if orig_dur else None, delta_color="inverse")
            rc2.metric("💾 ขนาดไฟล์ (MB)", f"{new_size:.1f} MB", f"{new_size - orig_size:.1f} MB" if orig_size else None, delta_color="inverse")
            rc3.metric("🖥️ ความละเอียดภาพ", f"{video_width} x {video_height}")
...
# =========================================================
    ...
    if uploaded_videos and st.button("🚀 รวมคลิปและสร้างซับไตเติล"):
        ...
        with st.spinner("กำลังฟังเสียงและถอดความ..."):
            with open("audio_joined.mp3", "rb") as audio_file:
                res_th = client.audio.transcriptions.create(
                    model="whisper-large-v3", 
                    file=("audio_joined.mp3", audio_file), 
                    response_format="verbose_json", 
                    language="th", 
                    temperature=0.0,
                    prompt="กรุณาถอดเสียงพูดภาษาไทยให้ถูกต้องแม่นยำ ห้ามพิมพ์ข้อความซ้ำกัน ห้ามแต่งประโยคเอง ถอดความตามเสียงที่ได้ยินเท่านั้น"
            ...
            for seg in segments_to_process:
                ...
                if chunk_size > 0 and len(lines) > chunk_size:
                    ...
                    ass += f"Dialogue: 0,{format_ass_timestamp(s)},{format_ass_timestamp(e)},Default,,0,0,0,,{effect_prefix}{formatted_text.replace('\n', '\\N')}\n"
        ...
        st.markdown("---")
        st.markdown("### 📊 สรุปข้อมูลคลิป (Video Report)")
        orig_size = get_file_size_mb("combined.mp4")
        orig_dur = get_video_duration("combined.mp4")
        new_size = get_file_size_mb("output_joined.mp4")
        new_dur = get_video_duration("output_joined.mp4")
        
        rc1, rc2, rc3 = st.columns(3)
        rc1.metric("⏱️ ความยาว (วินาที)", f"{new_dur:.1f} s", f"{new_dur - orig_dur:.1f} s" if orig_dur else None, delta_color="inverse")
        rc2.metric("💾 ขนาดไฟล์ (MB)", f"{new_size:.1f} MB", f"{new_size - orig_size:.1f} MB" if orig_size else None, delta_color="inverse")
        rc3.metric("🖥️ ความละเอียดภาพ", f"{v_w} x {v_h}")
