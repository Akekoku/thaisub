...
# 🌟 ย้อนกลับมาใช้ระบบจัดการซับที่เรียบง่ายและปลอดภัยที่สุด (V.28 Style)
def clean_whisper_segments(segments):
    ...
    for seg in segments:
        ...
        # กรองเฉพาะการพิมพ์ซ้ำแบบเป๊ะๆ 100% เท่านั้น จะไม่ไปรวบคำ หรือลบคำคล้ายทิ้งเด็ดขาด
...
# 🌟 ย้อนสมอง AI กลับไปใช้ Llama 3.1 (8B) พร้อมคำสั่งห้ามแตะต้องคำถ้าไม่จำเป็น (V.28 Style)
...
กฎเหล็กขั้นเด็ดขาด:
1. ห้ามเปลี่ยนความหมาย ห้ามลบคำ ห้ามหาทำแต่งประโยคใหม่เด็ดขาด (เช่น "ฟีเจอร์ลับๆ" ต้องคงไว้ ห้ามเปลี่ยนเป็น "ฟีเจอร์ๆ")
2. ถ้าข้อความต้นฉบับถูกต้องและอ่านรู้เรื่องอยู่แล้ว ให้ตอบกลับด้วยข้อความเดิม 100% ห้ามแก้ไขใดๆ ทั้งสิ้น
...
{json.dumps(data_to_fix, ensure_ascii=False)}"""
        ...
        for attempt in range(3):
            try:
                # 🌟 ถอยกลับมาใช้ Llama 3.1 8B เพื่อความซื่อตรง ไม่ล้ำเส้น
...
if app_mode == "🎭 โหมด 1: สร้างคลิปไร้หน้า (Faceless)":
    ...
    if uploaded_file and st.button("🚀 เริ่มผลิตคลิปไร้หน้า"):
        with st.spinner("กำลังทำงาน..."):
            ...
            # 🌟 ถอยกลับมาใช้ whisper-large-v3 ตัวเต็ม เพื่อความแม่นยำ 100% (แก้ปัญหา ขอบคุณ -> ขอบผุ่น)
            ...
            if sub_config["ai_proof"]:
                with st.spinner("🧠 กำลังให้ AI ตรวจทานคำผิดแบบอนุรักษ์นิยม..."):
            ...
            if faceless_mode == "ไม่ใช้ (ใช้วิดีโอต้นฉบับ)" and sub_config["auto_cut"] == "เปิด (ตัดช่วงเงียบอัตโนมัติ)":
                with st.spinner("✂️ กำลังตัดเดดแอร์แบบละเอียดระดับเฟรม (Frame-Accurate)..."):
                    ...
                    # 🌟 V.39 รักษาระบบ Outro Saver ไว้ให้คลิปไม่แหว่ง
    ...
    if uploaded_videos and st.button("🚀 รวมคลิปและสร้างซับไตเติล"):
        ...
        with st.spinner("กำลังฟังเสียงและถอดความ..."):
            with open("audio_joined.mp3", "rb") as audio_file:
                # 🌟 ถอยกลับมาใช้ whisper-large-v3 ตัวเต็ม
                res_th = client.audio.transcriptions.create(
                    model="whisper-large-v3", 
