...
# 🌟 ย้อนกลับมาใช้ระบบกรองคำที่เรียบง่ายและเสถียรที่สุด (V.28 Style)
def clean_whisper_segments(segments):
    ...
    for seg in segments:
        ...
        # กรองเฉพาะกรณีพูดซ้ำเป๊ะๆ ติดกัน (แก้ปัญหา Whisper พิมพ์ซ้ำเอง) โดยไม่ไปลบคำพูดจริงที่คล้ายกัน
        if text == last_text and (seg['start'] - last_seg['end'] < 0.5):
...
# 🌟 ย้อนกลับมาใช้สมอง AI พิสูจน์อักษรแบบมินิมอล ไม่คิดลึก ไม่เปลี่ยนคำเอง (V.28 Style)
def ai_proofread_segments(client, segments, user_replacements=""):
    ...
    if user_replacements:
        hint_text = f"\nคำใบ้เพิ่มเติม (ให้ความสำคัญกับการแก้คำเหล่านี้เป็นพิเศษ): {user_replacements}\n"

    for i in range(0, len(segments), chunk_size):
        chunk = segments[i:i+chunk_size]
        data_to_fix = [{"id": str(idx), "text": seg["text"]} for idx, seg in enumerate(chunk)]
        
        prompt = f"""คุณคือ AI ช่วยตรวจทานซับไตเติลภาษาไทย
หน้าที่ของคุณ: ตรวจสอบและแก้ไขคำผิดตาม "คำใบ้เพิ่มเติม" หรือแก้คำพ้องเสียงที่ผิดบริบทอย่างรุนแรงเท่านั้น

ข้อห้ามเด็ดขาด (Strict Rules):
1. ห้ามแต่งประโยคใหม่ ห้ามเปลี่ยนความหมายเด็ดขาด
2. ห้ามเปลี่ยนคำที่ถูกต้องอยู่แล้ว (เช่น ห้ามเปลี่ยน ร้อง เป็น ล้อง หรือ ลับ เป็น รับ)
3. หากประโยคถูกต้องอยู่แล้ว ให้ตอบกลับด้วยข้อความเดิม 100% ห้ามแก้ไขใดๆ ทั้งสิ้น
        ...
        with st.spinner("กำลังทำงาน..."):
            ...
            if faceless_mode == "ไม่ใช้ (ใช้วิดีโอต้นฉบับ)" and sub_config["auto_cut"] == "เปิด (ตัดช่วงเงียบอัตโนมัติ)":
                with st.spinner("✂️ กำลังตัดเดดแอร์แบบละเอียดระดับเฟรม (Frame-Accurate)..."):
                    ...
                    # 🌟 V.38 รักษาระบบ Outro Saver ไว้ให้คลิปไม่แหว่ง
