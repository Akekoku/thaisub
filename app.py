...
# 🌟 V.31 ฟังก์ชันใหม่: ตะแกรงกรองความหลอน (Anti-Stutter Filter)
def clean_whisper_segments(segments):
    cleaned = []
    for seg in segments:
        if not seg['text'].strip(): continue # ข้ามข้อความว่าง
        ...
        # 1. ป้องกัน AI พูดซ้ำ (ลบคำซ้ำและรวบเวลา)
        if seg['text'].strip() == last_seg['text'].strip():
            last_seg['end'] = max(last_seg['end'], seg['end'])
            continue
            
        # 2. ป้องกันเวลาย้อนกลับ (Time Paradox)
        if seg['start'] < last_seg['end']:
            seg['start'] = last_seg['end'] + 0.05 # ดันเวลาให้เดินหน้าเสมอ
            
        # ถ้าดันเวลาแล้วพัง (เวลาเริ่ม > เวลาจบ) ให้ข้ามบั๊กก้อนนี้ไปเลย
...
if app_mode == "🎭 โหมด 1: สร้างคลิปไร้หน้า (Faceless)":
    ...
    if uploaded_file and st.button("🚀 เริ่มผลิตคลิปไร้หน้า"):
        with st.spinner("กำลังทำงาน..."):
            ...
            # 🌟 บังคับ temperature=0.0 เพื่อลดอาการ AI หูแว่วตั้งแต่ต้นทาง
            ...
            # 🧹 รันระบบตะแกรงกรองความหลอนของ AI
            ...
            if faceless_mode == "ไม่ใช้ (ใช้วิดีโอต้นฉบับ)" and sub_config["auto_cut"] == "เปิด (ตัดช่วงเงียบอัตโนมัติ)":
                with st.spinner("✂️ กำลังตัดช่วงเงียบ (Jumpcut) อย่างทะนุถนอม..."):
                    jumpcut_content = ""
                    new_segments = []
                    curr_t = 0.0
                    last_e_cut = 0.0 # ป้องกันการข้ามเวลาย้อนหลัง
                    
                    for i, seg in enumerate(raw_segments):
                        pad_s = 0.1
                        pad_e = 0.1
                        if i > 0 and (seg['start'] - raw_segments[i-1]['end'] < 0.2): pad_s = (seg['start'] - raw_segments[i-1]['end']) / 2.0
                        if i < len(raw_segments) - 1 and (raw_segments[i+1]['start'] - seg['end'] < 0.2): pad_e = (raw_segments[i+1]['start'] - seg['end']) / 2.0
                        
                        s_cut = max(last_e_cut, seg['start'] - pad_s)
                        e_cut = max(s_cut + 0.1, seg['end'] + pad_e)
                        dur = e_cut - s_cut
                        
                        jumpcut_content += f"file '{video_to_process}'\ninpoint {s_cut}\noutpoint {e_cut}\n"
                        new_segments.append({"start": curr_t, "end": curr_t + dur, "text": seg['text']})
                        
                        curr_t += dur
                        last_e_cut = e_cut
                        
                    with open("jumpcut.txt", "w", encoding="utf-8") as f: f.write(jumpcut_content)
                    subprocess.run([FFMPEG_CMD, '-y', '-f', 'concat', '-safe', '0', '-i', 'jumpcut.txt', '-c:v', 'libx264', '-crf', '18', '-preset', 'fast', '-c:a', 'aac', '-b:a', '256k', 'jumpcut.mp4'], check=True)
