...
# 🌟 V.49 คืนชีพฟังก์ชันแก้สระลอยตัวเต็ม (แบบ V.28)
def fix_thai_floating_vowels(text):
    if not text: return text
    # 1. จัดระเบียบการพิมพ์สลับที่ด้วย PyThaiNLP
    text = normalize_thai(text)
    # 2. แก้ปัญหาสระอำ (แยก นิคหิต + ลากข้าง) เพื่อให้ FFmpeg เรนเดอร์ไม่ทับวรรณยุกต์
...
# 🌟 คืนชีพคำสั่งล้าง Cache ฟอนต์ในระบบจริงๆ แบบ V.28
st.sidebar.markdown("---")
st.sidebar.subheader("🛠️ เครื่องมือเสริม (Tools)")
enable_thai_fix = st.sidebar.checkbox("✅ เปิดระบบแก้สระลอยอัตโนมัติ", value=True, help="จัดระเบียบสระ/วรรณยุกต์ภาษาไทย (เช่น สระอำ) ให้แสดงผลบนวิดีโอได้ถูกต้อง")

if st.sidebar.button("🔄 รีเฟรชระบบเรนเดอร์สระลอย"):
    with st.spinner("กำลังเคลียร์แคชระบบฟอนต์ (fc-cache)..."):
        try:
            subprocess.run(["fc-cache", "-f", "-v"], check=False)
            font_cache_dir = os.path.expanduser("~/.cache/fontconfig")
            if os.path.exists(font_cache_dir):
                shutil.rmtree(font_cache_dir)
        except Exception:
            pass
    st.sidebar.success("✅ ล้างแคชฟอนต์ของระบบเรียบร้อยแล้ว!")
    time.sleep(1)
    st.rerun()
st.sidebar.markdown("---")
        ...
        if st.button("🎬 ขั้นตอนที่ 2: สร้างพรีวิววิดีโอ", use_container_width=True, type="primary"):
            with st.spinner("กำลังประกอบร่างวิดีโอ..."):
                ...
                for seg in segments_to_process:
                    ...
                    if enable_thai_fix: t = fix_thai_floating_vowels(t)
