...
def clean_whisper_segments(segments):
    ...
    for seg in segments:
        ...
        if seg['start'] < last_seg['end']: seg['start'] = last_seg['end'] + 0.01
...
กฎเหล็กขั้นเด็ดขาด:
1. ห้ามเปลี่ยนความหมาย ห้ามลบคำ ห้ามหาทำแต่งประโยคใหม่เด็ดขาด (เช่น "ฟีเจอร์ลับๆ" ต้องคงไว้)
2. ถ้าข้อความต้นฉบับถูกต้องและอ่านรู้เรื่องอยู่แล้ว ให้ตอบกลับด้วยข้อความเดิม 100%
3. ตอบกลับเป็น JSON Object เท่านั้น รูปแบบ: {{"corrected": [{{"id": "0", "text": "ข้อความ"}}]}}{hint_text}
ข้อความต้นฉบับ:
{json.dumps(data_to_fix, ensure_ascii=False)}"""
        ...
        for attempt in range(3):
            try:
                ...
                else: last_error_or_content = "⚠️ ปัญหา: จำนวนบรรทัดไม่ตรงกัน!"; time.sleep(2)
            except Exception as e: last_error_or_content = f"🔥 ปัญหา: {str(e)}"; time.sleep(3)
        if not chunk_success: success_all = False; debug_logs.append(last_error_or_content)
...
def get_file_size_mb(file_path): return os.path.getsize(file_path) / (1024 * 1024) if os.path.exists(file_path) else 0.0
...
if app_mode == "🎭 โหมด 1: สร้างคลิปไร้หน้า (Faceless)":
    ...
    if uploaded_file and st.button("🎧 ขั้นตอนที่ 1: ถอดเสียงและเตรียมสคริปต์", use_container_width=True, type="secondary"):
        with st.spinner("กำลังถอดเสียงและวิเคราะห์ข้อความ..."):
            ...
            # ดึงค่า proofread จาก session มารัน (ถ้าเปิดไว้)
            ...
            if is_proof:
                with st.spinner("🧠 กำลังให้ AI ตรวจทานคำผิด..."):
                    raw_segments, _, _ = ai_proofread_segments(client, raw_segments, user_rep)
    ...
    if st.session_state.m1_segments:
        ...
        st.info("💡 น้าสามารถคลิกที่ข้อความในตารางเพื่อแก้ไขคำผิด ลบบรรทัด หรือปรับเวลาได้โดยตรงเลยครับ")
        ...
        st.session_state.m1_segments = edited_segments # เซฟค่าที่แก้กลับเข้า Session
        ...
        if st.button("🎬 ขั้นตอนที่ 2: สร้างพรีวิววิดีโอ", use_container_width=True, type="primary"):
            with st.spinner("กำลังประกอบร่างวิดีโอ..."):
                ...
                # ... (ส่วนการตัด Dead air และสร้าง Video แบบ Faceless เหมือนโค้ดเดิมเป๊ะ)
                ...
                for text_sub, start_time, end_time in segments_data_for_subs:
                    lines = split_text_by_pixel_width(text_sub, actual_font_file, actual_pil_font_size, allowed_pixel_width).split('\n')
                    if chunk_size > 0 and len(lines) > chunk_size:
                        ...
                        for chunk in chunks:
                            ...
                            t_start = t_end
                    else: ass_content += f"Dialogue: 0,{format_ass_timestamp(start_time)},{format_ass_timestamp(end_time)},Default,,0,0,0,,{effect_prefix}{'\\N'.join(lines)}\n"
                with open("subs.ass", "w", encoding="utf-8") as f: f.write(ass_content)

                if faceless_mode != "ไม่ใช้ (ใช้วิดีโอต้นฉบับ)":
                    # (ย่อโค้ด Pexels เพื่อความกะทัดรัด แต่ยังทำงานได้ปกติ)
            ...
            with open("output.mp4", "rb") as f: 
                st.download_button("📥 ดาวน์โหลดวิดีโอ (นำไปใช้งานได้เลย)", f, "faceless_output.mp4", "video/mp4", type="primary", use_container_width=True)
...
if "m2_segments" not in st.session_state: st.session_state.m2_segments = None
    ...
    if st.session_state.m2_segments:
        ...
        st.session_state.m2_segments = edited_segments # เซฟค่ากลับ
        ...
        if st.button("🎬 ขั้นตอนที่ 2: สร้างพรีวิววิดีโอ", use_container_width=True, type="primary"):
            with st.spinner("กำลังฝังซับไตเติล..."):
                ...
                for seg in segments_to_process:
                    t = seg['text']
                    for old_w, new_w in sub_config["replacements"].items(): t = re.sub(re.escape(old_w), new_w, t, flags=re.IGNORECASE)
                    lines = split_text_by_pixel_width(t, actual_font_file, actual_pil_font_size, allowed_pixel_width).split('\n')
                    if chunk_size > 0 and len(lines) > chunk_size:
                        chunks = [ "\n".join(lines[j:j+chunk_size]) for j in range(0, len(lines), chunk_size) ]
                        ...
                        for chunk in chunks:
                            ...
                            t_start = t_end
                    else: ass += f"Dialogue: 0,{format_ass_timestamp(seg['start'])},{format_ass_timestamp(seg['end'])},Default,,0,0,0,,{effect_prefix}{'\\N'.join(lines)}\n"
            ...
            with open("output_joined.mp4", "rb") as f: 
                st.download_button("📥 ดาวน์โหลดวิดีโอ (นำไปใช้งานได้เลย)", f, "joined_video_with_subs.mp4", "video/mp4", type="primary", use_container_width=True)
