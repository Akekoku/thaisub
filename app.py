...
# 🌟 ฟังก์ชันใหม่ V.17: AI ตรวจคำผิดแบบบังคับตอบเป็น JSON
...
ให้ถูกต้องตามบริบทของประโยค ห้ามเปลี่ยนความหมายและห้ามเปลี่ยนสไตล์การพูด
    
คำสั่งบังคับสูงสุด: คุณต้องตอบกลับเป็นรูปแบบ JSON Object เท่านั้น ห้ามมีข้อความเกริ่นนำหรือคำอธิบายใดๆ ทั้งสิ้น
ให้ใช้ key ชื่อ "corrected" และ value เป็น Array ของข้อความที่แก้ไขแล้ว
ตัวอย่างรูปแบบที่ถูกต้อง: {{"corrected": ["ข้อความบรรทัด 1", "ข้อความบรรทัด 2"]}}
...
"""
    ...
    try:
        res = client.chat.completions.create(
            ...
            temperature=0.1,
            response_format={"type": "json_object"}  # บังคับตอบเป็น JSON 100%
        )
        ...
        # Safety Fallback: ต้องมีจำนวนบรรทัดเท่าเดิมเป๊ะ
        if len(corrected_lines) == len(lines_to_fix):
            for i in range(len(segments)):
                segments[i]["text"] = str(corrected_lines[i])
        ...
        with st.spinner("กำลังทำงาน..."):
            ...
            if sub_config["ai_proof"]:
                with st.spinner("🧠 กำลังให้ AI (Llama-70b) ตรวจทานและแก้คำผิด..."):
                    ...
                    else: st.toast("⚠️ ตรวจพบ AI นอกเรื่อง! ระบบสลับไปใช้ข้อความเดิมเพื่อความปลอดภัย")
