...
# 🌟 V.47 แก้ไขบั๊กซับหาย: เปลี่ยนจากการลบทิ้ง เป็นการดันเวลาหนีแทน
def clean_whisper_segments(segments):
    ...
    for seg in segments:
        ...
        if seg['start'] < last_seg['end']: 
            orig_dur = max(0.5, seg['end'] - seg['start']) # จำความยาวเดิมไว้ (อย่างน้อยครึ่งวิ)
            seg['start'] = last_seg['end'] + 0.01 # ดันเวลาเริ่มไปต่อท้ายอันเก่า
            seg['end'] = seg['start'] + orig_dur # ดันเวลาจบตามไปด้วย จะได้ไม่โดนลบ
...
if app_mode == "🎭 โหมด 1: สร้างคลิปไร้หน้า (Faceless)":
    ...
    if st.session_state.m1_segments:
        ...
        st.info("💡 น้าสามารถกดแถวว่างด้านล่างสุดของตาราง เพื่อพิมพ์เพิ่มข้อความหรือเวลาที่ขาดหายไปได้เลยครับ")
