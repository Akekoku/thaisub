...
# 🌟 V.32 ฟังก์ชันกรองความหลอน AI ให้แม่นยำขึ้น
def clean_whisper_segments(segments):
    ...
    for seg in segments:
        ...
        # 1. ข้อความซ้ำและเวลาทับซ้อนกัน ให้รวบเป็นอันเดียว (AI พิมพ์ย้ำ)
        if seg['text'].strip() == last_seg['text'].strip() and seg['start'] < last_seg['end']:
            last_seg['end'] = max(last_seg['end'], seg['end'])
            continue
            
        # 2. เวลาทับซ้อนกันแบบน่าเกลียดเกิน 0.5 วินาที (AI หลอนประโยคแทรก) ข้ามไปเลย
        overlap = max(0, min(last_seg['end'], seg['end']) - max(last_seg['start'], seg['start']))
        if overlap > 0.5:
            continue
            
        # 3. ขยับเวลาให้ไหลลื่นถ้าเหลื่อมกันเล็กน้อย
...
if app_mode == "🎭 โหมด 1: สร้างคลิปไร้หน้า (Faceless)":
    ...
    if uploaded_file and st.button("🚀 เริ่มผลิตคลิปไร้หน้า"):
        with st.spinner("กำลังทำงาน..."):
            ...
            # 🌟 V.32 Frame-Accurate Jumpcut System (โหมดใช้วิดีโอต้นฉบับ) 🌟
            if faceless_mode == "ไม่ใช้ (ใช้วิดีโอต้นฉบับ)" and sub_config["auto_cut"] == "เปิด (ตัดช่วงเงียบอัตโนมัติ)":
                with st.spinner("✂️ กำลังตัดเดดแอร์แบบละเอียดระดับเฟรม (Frame-Accurate)..."):
                    ...
                    if merged_times:
                        ...
                        # รีเซ็ตเวลาซับไตเติลใหม่ให้ตรงกับวิดีโอที่ถูกตัดแล้ว
                        new_segments = []
...
# =========================================================
    ...
    if uploaded_videos and st.button("🚀 รวมคลิปและสร้างซับไตเติล"):
        ...
        with st.spinner("กำลังฟังเสียงและถอดความ..."):
            ...
            # 🌟 V.32 Frame-Accurate Jumpcut System (โหมด Join & Sub) 🌟
