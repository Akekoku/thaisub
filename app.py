...
# 🌟 ฟังก์ชันอัปเกรด V.22: ระบบแจ้งเตือนข้อผิดพลาด (Debug Logs) 
def ai_proofread_segments(client, segments):
    chunk_size = 10
    success_all = True
    debug_logs = [] # 📝 เก็บหลักฐานความผิดของ AI
...
{json.dumps(data_to_fix, ensure_ascii=False)}"""
        ...
        for attempt in range(3):
            try:
                ...
                last_error_or_content = content # เก็บหลักฐานก่อนลอกคราบ
                ...
                if isinstance(corrected_list, list) and len(corrected_list) == len(data_to_fix):
                    ...
                    last_error_or_content = f"⚠️ ปัญหา: จำนวนบรรทัดไม่ตรงกัน!\nส่งไป: {len(data_to_fix)} บรรทัด\nตอบกลับมา: {len(corrected_list)} บรรทัด\n\nข้อความดิบที่ AI ส่งมา:\n{content}"
                    time.sleep(2)
            except Exception as e:
                last_error_or_content = f"🔥 ปัญหา: โค้ดพัง หรือ เซิร์ฟเวอร์ล่ม!\nError: {str(e)}\n\nข้อความดิบที่พยายามอ่าน:\n{last_error_or_content}"
                time.sleep(3)
                
        if not chunk_success:
            success_all = False
            debug_logs.append(last_error_or_content) # โยนหลักฐานเข้าแฟ้ม
...
def render_subtitle_ui(key_prefix):
    ...
    with st.expander("เปิดเครื่องมือปรับแต่ง (ฟอนต์, เอฟเฟกต์, AI)", expanded=True):
        ...
        ai_proof_choice = st.checkbox("เปิดใช้งาน AI พิสูจน์อักษร (ฉลาดมาก แต่อาจใช้เวลาประมวลผลเพิ่มเล็กน้อย)", key=f"{key_prefix}_ai_proof")
        st.markdown("---")
...
if app_mode == "🎭 โหมด 1: สร้างคลิปไร้หน้า (Faceless)":
    ...
    if uploaded_file and st.button("🚀 เริ่มผลิตคลิปไร้หน้า"):
        with st.spinner("กำลังทำงาน..."):
            ...
            # 🛠️ โชว์ Error Logs ตรงนี้เลย!
            if sub_config["ai_proof"]:
                with st.spinner("🧠 กำลังให้ AI ตรวจทานและแก้คำพ้องเสียง (รอสักครู่)..."):
                    raw_segments, ai_success, debug_logs = ai_proofread_segments(client, raw_segments)
                    if ai_success: 
                        st.toast("✨ AI แก้ไขคำผิดตามบริบทเรียบร้อยแล้ว!")
                    else: 
                        st.toast("⚠️ มีข้อผิดพลาดบางส่วน! ระบบสำรองข้อความเดิมมาช่วยแล้ว")
                        st.error("🚨 [โหมดแฉ AI] พบการตอบกลับที่ผิดพลาดจากเซิร์ฟเวอร์:")
                        for idx, log in enumerate(debug_logs):
                            with st.expander(f"🔍 ดูข้อความดิบที่ AI ตอบกลับมา (จุดที่ Error ที่ {idx+1})", expanded=True):
