...
# 🌟 V.34 ปรับระบบกรองคำหลอนให้ฉลาดขึ้น ไม่กินคำพูดเร็ว
def clean_whisper_segments(segments):
    ...
    for seg in segments:
        ...
        # ลดความแข็งของ Filter เพื่อไม่ให้ลบคำพูดจริงที่พูดไวๆ (จะลบต่อเมื่อเหมือนกันเป๊ะ 85% ขึ้นไป)
...
if app_mode == "🎭 โหมด 1: สร้างคลิปไร้หน้า (Faceless)":
    ...
    if uploaded_file and st.button("🚀 เริ่มผลิตคลิปไร้หน้า"):
        with st.spinner("กำลังทำงาน..."):
            ...
            # 🌟 V.34 เปลี่ยนใช้โมเดล turbo และเอา Prompt เจ้าปัญหาออก
            ...
            # เก็บความยาววิดีโอต้นฉบับไว้เพื่อปกป้องฉากจบ
            ...
            if faceless_mode == "ไม่ใช้ (ใช้วิดีโอต้นฉบับ)" and sub_config["auto_cut"] == "เปิด (ตัดช่วงเงียบอัตโนมัติ)":
                with st.spinner("✂️ กำลังตัดเดดแอร์แบบละเอียดระดับเฟรม (Frame-Accurate)..."):
                    ...
                    # 🌟 V.34 ป้องกันการตัด Outro / CTA ท้ายคลิปทิ้ง
                    if merged_times and orig_dur_process > 0:
                        # ถ้าระยะเวลาคำพูดสุดท้ายห่างจากจุดจบวิดีโอไม่เกิน 4 วินาที ให้สั่งลากยาวคลิปไปจนจบเลย
    ...
    if uploaded_videos and st.button("🚀 รวมคลิปและสร้างซับไตเติล"):
        ...
        with st.spinner("กำลังฟังเสียงและถอดความ..."):
            with open("audio_joined.mp3", "rb") as audio_file:
                # 🌟 V.34 เปลี่ยนใช้โมเดล turbo และเอา Prompt เจ้าปัญหาออกเช่นกัน
                res_th = client.audio.transcriptions.create(
                    model="whisper-large-v3-turbo", 
