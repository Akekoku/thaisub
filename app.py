...
# ฟังก์ชันแปลงโค้ดสี Hex (#FFFFFF) ให้เป็นรูปแบบเฉพาะของไฟล์ ASS (&H00BBGGRR)
...
# ฟังก์ชันแปลงเวลาเป็นรูปแบบไฟล์ ASS (H:MM:SS.cs)
...
st.title("🎬 ระบบฝังซับอัตโนมัติเวอร์ชันเอฟเฟกต์เด้งพอง")
...
with st.expander("คลิกเพื่อเปิดเครื่องมือปรับแต่งตัวอักษร สี และเอฟเฟกต์ Kinetic", expanded=True):
    ...
    # =========================================================
    # 🌟 ฟีเจอร์ใหม่: แผงควบคุมเอฟเฟกต์เด้งพองตามจังหวะไฮไลต์
    # =========================================================
    st.markdown("#### 💥 เอฟเฟกต์การเคลื่อนไหว (Kinetic Animation)")
    enable_pop = st.toggle("🔥 เปิดเอฟเฟกต์ตัวหนังสือเด้งพอง (Pop-up Punch)", value=True)
    if enable_pop:
...
if uploaded_file and api_key:
    if st.button("🚀 เริ่มกระบวนการสร้างซับและฝังวีดีโอ"):
        ...
        try:
            ...
            # แปลงสีหน้าเว็บเป็นโค้ดสีระบบ ASS
            primary_color_ass = hex_to_ass_color(text_color)
            outline_color_ass = hex_to_ass_color(outline_color)
            
            # ถ้าเลือกเป็นกล่องดำ ให้ตั้งค่าสีพื้นหลังกล่อง (BackColour) เป็นสีดำจางๆ
            ...
            # สร้างโครงสร้างหัวไฟล์ซับไตเติลขั้นสูง (ASS Header) เพื่อล็อกความชัดพิกเซลตรงกับวิดีโอ
...
Style: Default,{font_choice},{font_size_choice},{primary_color_ass},&H0000FFFF,{outline_color_ass},{back_color_ass},0,0,0,0,100,100,0,0,{border_style},{outline_thickness},0,2,10,10,{margin_v_choice},1
...
"""
            ...
            for i, segment in enumerate(response.segments, start=1):
                ...
                # ในระบบไฟล์ ASS การขึ้นบรรทัดใหม่จะใช้สัญลักษณ์ \N (N ใหญ่)
                formatted_text_ass = formatted_text.replace("\n", "\\N")
                
                # 💥 ถ้าเปิดใช้งานระบบเด้งพอง ให้แทรกโค้ดสั่ง Animation ขยายพิกเซลต้นประโยค
                if enable_pop:
                    # \fscx และ \fscy คือการขยายขนาดกว้างยาวเริ่มต้น, \t คือคำสั่งทำ Animation บีบกลับมาที่ 100%
        ...
        try:
            ...
            # เรียกใช้ไฟล์ subs.ass ในการฝังแทน srt
            cmd = [
                'ffmpeg', '-y',
                '-i', 'input.mp4',
