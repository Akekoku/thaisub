...
# 🌟 ฟังก์ชันแก้สระลอยที่สมบูรณ์ (คืนชีพสูตรแยกสระอำ)
def fix_thai_floating_vowels(text):
    if not text: return text
    text = normalize_thai(text)
    text = text.replace("ำ", "ํา") # แยกนิคหิตกับลากข้าง ป้องกันสระลอยทับวรรณยุกต์ตอนใช้ FFmpeg
    return text

# 🌟 ฟังก์ชันติดตั้งเอนจินอัตโนมัติ (แก้ปัญหาเซิร์ฟเวอร์ลืมไฟล์ตอนเล่นบนมือถือ)
def ensure_ffmpeg_engine():
    if not (os.path.exists("./ffmpeg") and os.path.exists("./ffprobe")):
        with st.spinner("⏳ [ระบบอัตโนมัติ] กำลังดาวน์โหลดเอนจินแก้สระลอย (ทำครั้งเดียว)..."):
            try:
                ...
                for root, dirs, files in os.walk("ffmpeg_temp"):
                    if "ffmpeg" in files and "ffprobe" in files:
                        shutil.copy(os.path.join(root, "ffmpeg"), "./ffmpeg")
                        shutil.copy(os.path.join(root, "ffprobe"), "./ffprobe")
                        break
                ...
                st.error(f"❌ เกิดข้อผิดพลาดในการโหลดเอนจิน: {e}")
...
# แถบแจ้งเตือนด้านซ้าย
st.sidebar.markdown("---")
st.sidebar.markdown("**TH เครื่องมือแก้สระลอย (ทำงานอัตโนมัติ)**")
engine_exists = os.path.exists("./ffmpeg") and os.path.exists("./ffprobe")
if engine_exists:
    st.sidebar.success("✅ ระบบพร้อมเรนเดอร์สระภาษาไทยแบบเป๊ะๆ 100%")
else:
    st.sidebar.info("💡 ระบบจะติดตั้งเอนจินอัตโนมัติเมื่อกดปุ่มเรนเดอร์")

st.sidebar.markdown("---")

def get_video_dimensions(video_path):
    try:
        cmd = ["./ffprobe" if os.path.exists("./ffprobe") else "ffprobe", '-v', 'error', '-select_streams', 'v:0', '-show_entries', 'stream=width,height', '-of', 'csv=s=x:p=0', video_path]
...
def get_video_duration(video_path):
    try:
        cmd = ["./ffprobe" if os.path.exists("./ffprobe") else "ffprobe", '-v', 'error', '-show_entries', 'format=duration', '-of', 'default=noprint_wrappers=1:nokey=1', video_path]
...
if app_mode == "🎭 โหมด 1: สร้างคลิปไร้หน้า (Faceless)":
    ...
    if uploaded_file and st.button("🎧 ขั้นตอนที่ 1: ถอดเสียงและเตรียมสคริปต์", use_container_width=True, type="secondary"):
        with st.spinner("กำลังถอดเสียงและวิเคราะห์ข้อความ..."):
            ...
            elif file_ext == "wav": subprocess.run(["./ffmpeg" if os.path.exists("./ffmpeg") else "ffmpeg", '-y', '-i', raw_path, '-c:a', 'libmp3lame', 'audio.mp3'], check=True)
            else: subprocess.run(["./ffmpeg" if os.path.exists("./ffmpeg") else "ffmpeg", '-y', '-i', raw_path, '-vn', '-c:a', 'libmp3lame', 'audio.mp3'], check=True)
    ...
    if st.session_state.m1_segments:
        ...
        if st.button("🎬 ขั้นตอนที่ 2: สร้างพรีวิววิดีโอ", use_container_width=True, type="primary"):
            ...
            # 🌟 อัปเกรด V.52: ติดตั้งเอนจินอัตโนมัติก่อนเรนเดอร์!
            ...
            with st.spinner("กำลังประกอบร่างวิดีโอ..."):
                ...
                for seg in segments_to_process:
                    ...
                    # 🌟 ใช้ฟังก์ชันแก้สระลอยที่สมบูรณ์
...
if "m2_segments" not in st.session_state: st.session_state.m2_segments = None
    ...
    if uploaded_videos and st.button("🎧 ขั้นตอนที่ 1: รวมคลิปและถอดเสียง", use_container_width=True, type="secondary"):
        with st.spinner("กำลังต่อคลิปวิดีโอและถอดเสียง..."):
            ...
            FFMPEG_CMD_TEMP = "./ffmpeg" if os.path.exists("./ffmpeg") else "ffmpeg"
            subprocess.run([FFMPEG_CMD_TEMP, '-y', '-f', 'concat', '-safe', '0', '-i', 'concat_join.txt', '-c', 'copy', 'combined.mp4'], check=True)
            subprocess.run([FFMPEG_CMD_TEMP, '-y', '-i', 'combined.mp4', '-vn', '-c:a', 'libmp3lame', 'audio_joined.mp3'], check=True)
    ...
    if st.session_state.m2_segments:
        ...
        if st.button("🎬 ขั้นตอนที่ 2: สร้างพรีวิววิดีโอ", use_container_width=True, type="primary"):
            ...
            # 🌟 อัปเกรด V.52: ติดตั้งเอนจินอัตโนมัติก่อนเรนเดอร์เช่นกัน!
            ensure_ffmpeg_engine()
