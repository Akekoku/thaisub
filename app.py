...
st.markdown("""
    ...
    .stTabs [aria-selected="true"] { background-color: #e0e4eb; font-weight: bold; border-bottom: 2px solid #ff4b4b; }
    </style>
...
if not st.session_state["authenticated"]:
    ...
    with col2:
        ...
        st.markdown("<p style='text-align: center;'>กรุณายืนยันตัวตนเพื่อเข้าสู่ AI Studio Pro</p>", unsafe_allow_html=True)
...
if os.path.exists("./ffmpeg") and os.path.exists("./ffprobe"):
    st.sidebar.success("✅ เอนจินพร้อมใช้งาน")
else:
    st.sidebar.warning("⚠️ ยังไม่มีเอนจิน โปรดติดตั้ง")

if st.sidebar.button("🔄 อัปเกรดเอนจิน (ซ่อมสระลอย)", use_container_width=True):
...
def render_subtitle_ui(key_prefix):
    ...
    with st.expander("⚙️ แผงควบคุมและตั้งค่าสตูดิโอ (คลิกเพื่อเปิด/ปิด)", expanded=False):
        ...
        with tab_design:
            ...
            line_limit_choice = st.radio("การจัดบรรทัด", line_options, horizontal=True, key=f"{key_prefix}_ll")
        ...
        with tab_profile:
            ...
            st.markdown("---")
            st.markdown("#### ☁️ สำรองข้อมูลโปรไฟล์")
            col_exp, col_imp = st.columns(2)
            with col_exp:
                if os.path.exists(PROFILE_FILE):
                    with open(PROFILE_FILE, "rb") as f:
                        st.download_button("📤 ดาวน์โหลดไฟล์โปรไฟล์", f, file_name="my_profiles.json", mime="application/json", key=f"{key_prefix}_export", use_container_width=True)
            with col_imp:
                uploaded_prof = st.file_uploader("📥 นำเข้าไฟล์โปรไฟล์ (.json)", type=["json"], key=f"{key_prefix}_import", label_visibility="collapsed")
                if uploaded_prof and st.button("อัปเดตเข้าระบบ", key=f"{key_prefix}_do_import", use_container_width=True):
                    try:
                        imported_data = json.loads(uploaded_prof.getvalue().decode("utf-8"))
                        profiles.update(imported_data)
                        save_profiles(profiles)
                        st.success("นำเข้าสำเร็จ!"); time.sleep(1); st.rerun()
        ...
        return {
            ...
            "pop_scale": pop_scale, "pop_dur": pop_duration, 
            "fade_dur": fade_duration, "size": font_size_choice, 
...
if app_mode == "🎭 โหมด 1: สร้างคลิปไร้หน้า (Faceless)":
    ...
    st.markdown("### 1️⃣ อัปโหลดและเตรียมไฟล์")
    ...
    st.markdown("### 2️⃣ ปรับแต่งผลลัพธ์")
    sub_config = render_subtitle_ui("mode1")

    st.markdown("### 3️⃣ สั่งผลิตผลงาน")
    if uploaded_file and st.button("🚀 เริ่มผลิตคลิปไร้หน้า", use_container_width=True, type="primary"):
        with st.spinner("กำลังทำงาน..."):
            ...
            if faceless_mode == "ไม่ใช้ (ใช้วิดีโอต้นฉบับ)" and sub_config["auto_cut"] == "เปิด (ตัดช่วงเงียบอัตโนมัติ)":
                with st.spinner("✂️ กำลังตัดเดดแอร์..."):
                    merged_times = []
        ...
        st.success("🎉 ผลิตคลิปเสร็จสมบูรณ์!")
        ...
        with open("output.mp4", "rb") as f: st.download_button("📥 ดาวน์โหลดวิดีโอผลลัพธ์", f, "faceless_output.mp4", "video/mp4", type="primary")
...
# =========================================================
    ...
    st.markdown("### 3️⃣ สั่งผลิตผลงาน")
    if uploaded_videos and st.button("🚀 รวมคลิปและสร้างซับไตเติล", use_container_width=True, type="primary"):
        ...
        with open("output_joined.mp4", "rb") as f: st.download_button("📥 ดาวน์โหลดวิดีโอผลลัพธ์", f, "joined_video_with_subs.mp4", "video/mp4", type="primary")
