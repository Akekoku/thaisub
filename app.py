...
# 🌟 ฟังก์ชันใหม่: AI ตรวจคำผิดแบบ 100% Failsafe
def ai_proofread_segments(client, segments):
    lines_to_fix = [seg["text"] for seg in segments]
    prompt = "คุณคือผู้เชี่ยวชาญพิสูจน์อักษรภาษาไทย หน้าที่ของคุณคือแก้ไขคำผิดที่เกิดจากการฟังผิด (เช่น รถ/ลด, น่า/หน้า) ในประโยคด้านล่างให้ถูกต้องตามบริบท \nข้อบังคับสูงสุด: ต้องคืนค่ากลับมาจำนวนบรรทัดเท่าเดิมเป๊ะๆ ห้ามรวมบรรทัด ห้ามเพิ่มข้อความพูดคุยเด็ดขาด ให้ตอบมาเฉพาะบรรทัดที่แก้ไขแล้วเท่านั้น\n\n" + "\n".join(lines_to_fix)
    ...
    try:
        ...
        # คลีนข้อความเผื่อ AI ใส่ - หรือ 1. นำหน้ามาให้
        cleaned_lines = [re.sub(r'^[-*]\s*|^\d+\.\s*', '', line) for line in corrected_lines]

        # Safety Fallback: ถ้าจำนวนบรรทัดเท่าเดิม ถือว่าผ่าน! เอาไปแทนที่ได้
        if len(cleaned_lines) == len(lines_to_fix):
            for i in range(len(segments)):
                segments[i]["text"] = cleaned_lines[i]
            return segments, True
        else:
            # ถ้าไม่เท่าเดิม ตีกลับไปใช้ของเดิมทันที ป้องกันคลิปพัง!
...
def render_subtitle_ui(key_prefix):
    defaults = {
        "replace": "Save=เซฟ, OK=โอเค", "font": "Sarabun", "tc": "#FFFFFF", "oc": "#000000", "bg": "ขอบปกติ",
    ...
    with st.expander("เปิดเครื่องมือปรับแต่ง (ฟอนต์, เอฟเฟกต์, AI)", expanded=True):
        ...
        # 🌟 สวิตช์เปิด-ปิด AI ตรวจคำผิด 
        st.markdown("#### 🧠 ผู้ช่วย AI ตรวจคำผิดตามบริบท")
        ai_proof_choice = st.checkbox("เปิดใช้งาน AI พิสูจน์อักษร (ฉลาดมาก แต่อาจใช้เวลาประมวลผลเพิ่ม 5-10 วินาที)", key=f"{key_prefix}_ai_proof")
        st.markdown("---")
        
        st.markdown("#### 📝 แก้ไขคำทับศัพท์ / เปลี่ยนคำผิด (บังคับคำ)")
...
if app_mode == "🎭 โหมด 1: สร้างคลิปไร้หน้า (Faceless)":
    ...
    if uploaded_file and st.button("🚀 เริ่มผลิตคลิปไร้หน้า"):
        with st.spinner("กำลังทำงาน..."):
            ...
            # แปลงข้อมูลให้อยู่ในรูปแบบ Dict ก่อน
            raw_segments = []
            ...
            # 🧠 ใช้งาน AI Proofreader ถ้าเปิดสวิตช์
            if sub_config["ai_proof"]:
                with st.spinner("🧠 กำลังให้ AI (Llama-70b) ตรวจทานและแก้คำผิด..."):
                    raw_segments, ai_success = ai_proofread_segments(client, raw_segments)
                    if ai_success: st.toast("✨ AI แก้ไขคำผิดตามบริบทเรียบร้อยแล้ว!")
                    else: st.toast("⚠️ AI ตอบกลับผิดพลาด ระบบสลับไปใช้ข้อความเดิมเพื่อความปลอดภัย")
